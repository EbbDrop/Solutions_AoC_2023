...
parse_not_new_line:
        ...
        lbu t0, 0(s1)       # load char
        li t1, '*'
        bne t0, t1, parse_not_symbol
        sw s1, 0(s5)        # Store symbol position
        addi s5, s5, 8      # move symbols cursor, leaving space for flags
...
number_loop_reset:
        ...
        blt t5, zero, number_loop_reset_end    # end of number when flags is -1
        sw zero, -8(s2)         # reste the flags
indexes_loop_reset:               # loop to skip to -1
        lw t3, 0(s2)
        addi s2, s2, 4       # move scratch passed index
        blt t3, zero, number_loop_reset    # end of indexes when index is -1
        j indexes_loop_reset
number_loop_reset_end:
# done reset number flags

        lw t0, 0(s5)         # symbol position
        beq t0, zero, symbol_loop_end
        addi s5, s5, 8

        li a5, 0             # amount of numbers
        li a6, 1             # ratio
...
indexes_loop:
        ...
        blt zero, t5, indexes_loop         # dont do calucalations if visited flag is set
        sub t4, t3, t1       # t4 = index - to look for index
        seqz t4, t4          # t4 = 1 if index is the to look for index
        beq t4, zero, indexes_loop # not what we are looking for
        addi a5, a5, 1       # found one more number
        lw a7, 4(t6)         # the value
        mul a6, a6, a7
...
number_loop_end:
        ...
        li a4, 32
        blt t0, a4, position_loop
# went over all positions
        li a4, 2
        bne a5, a4, not_a_gear
        add s0, s0, a6
not_a_gear:
